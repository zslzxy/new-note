...
![1590333856900](assets/1590333856900.png)

> ​	`JVM`中的**程序计数寄存器(Program Counter Register)** 中，`Register`是参照于CPU的寄存器，寄存器存储质量相关的现场信息；CPU只有将数据装载到寄存器中才能够运行；
>
> ​	JVM中的寄存器可将其翻译为指令计数器或者是程序钩子，相当于是实际的物理寄存器的一种抽象概念，作用于操作系统之上的JVM系统；

> 作用：
>
> ​	**PC寄存器是用于存储指向下一条指令的地址，也即将要执行的指令代码；由执行引擎读取下一条指令；**
>
> - 它his程序控制流的指示器，分支、循环、跳转、异常处理、线程恢复等基础功能都是需要依赖这个计数器来完成的；
> - 字节码解释器工作时就是通过改变这个计数器的值来获取下一条需要执行的字节码指令；
> - 它是唯一一个在java虚拟机规范中没有设置`OutOfMemoryError`的区域；
>
> 执行流程：
>
> - 第一步：java栈中存放着一个一个的栈帧，每一个栈帧对应着都是一个方法，所以每个栈帧中存在着对应的每个局部变量表、操作数栈等；
> - 第二步：使用pc寄存器保存着对应的栈帧中的指令位置；
> - 第三步：执行引擎从pc寄存器中拉取到了对应的栈帧指令位置，根据该指令位置执行对应的代码；
> - 第四步：更新pc寄存器指向的位置；
>
> ![1590334225190](assets/1590334225190.png)

### 2.2 虚拟机栈

> **栈是运行时单位， 而堆是存储的单位；**
>
> 虚拟机栈（Java Virtual Machine Stack），也称Java栈；每个线程在创建时都会创建一个虚拟机栈，其内部保存一个个的栈帧(`Stack Frame`)，对应着一个个Java方法的调用；
>
> 虚拟机栈的生命周期与线程一致；
>
> 主要用于管理Java的运行，在虚拟机栈中，一个方法就是一个栈帧，而一个栈帧就是虚拟机栈中的一个对象，一个对象中保存的是局部变量表、动态链接、方法返回值等参数；
>
> 虚拟机栈的整个过程是会获取栈顶元素，翻译过来就是获取需要执行的第一个方法，执行完毕以后出栈，再执行后续方法；

> **Java虚拟机规范允许Java虚拟机栈的大小是动态或者固定不变的；**
>
> 栈可能出现的异常：
>
> `StackOverFlowError`：如果采用固定大小的Java虚拟机栈，那么当线程请求分配的栈的容量超过了虚拟机栈允许的最大容量，则抛出该异常；
>
> `OutOfMemoryError`：如果Java虚拟机栈可以动态扩展，并且尝试扩展时无法申请到足够的内存，或者在创建的线程时没有足够的内存去创建对应的虚拟机栈，将会抛出该异常；